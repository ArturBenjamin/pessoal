...
int main(int argc, char *argv[])
{
    ...
    // Initialize game
    g = upecman_init();
    //define o tempo inicial para zero
...
void game_loop(t_game *g) {
    ...
    while(game_running) {
        ...
        else if(g->cereja_timer.elapsed_seconds >= 10 && g->cereja_apareceu == 1)
        {
            cereja_desapareceu(g);
...
//Inky Mov
t_game inkymove(t_game g, int elepsed_seconds)
{
    if(elepsed_seconds < g.ghost[2].start_time)
        return g;// Inky não se move antes do tempo inicial
...
//Blinky modos: chase,scatter,dead,afraid

t_game modosfantasmas(t_game g) // os 4 modos dos fantasmas
{
    int i;
    float dx, dy;
        ...
        switch(i)
        {
            case(blinky):
            {
                ...
                if(g.ghost[i].mode == dead)
                {
                    g.ghost[i].pos.y = 10;
                    g.ghost[i].pos.x = 10;
                    h.ghost[i].mode = chase;
                    continue;
                }
                if(g.ghost[i].mode == afraid)
                {

                    h.ghost[i].starget.y = rand() % 20;
                    h.ghost[i].starget.x = rand() % 20;
                }
                break;
            }
        }
...
//Clyde modos: chase,sactter,dead,afraid
  int i;
    float dx, dy;
    ...
    for(i = blinky; i <= clyde; i++)
    {
        switch(i)
            ...
            case(clyde):
            {
                ...
                {
                    g.ghost[i].pos.y--; // Sobe para sair da casa
                    continue; // Pula para o próximo fantasma se ainda estiver na casa
                }
                // Adicione a condição para ele se mover para a direita para sair da casa,
                // se for o caso (ex: g.ghost[i].pos.y == 7 && g.ghost[i].pos.x < 10)
                // Se a posição da casa for (10,10) ele só sobe até y=7 e já está no corredor.


                // Lógica para os modos de Clyde
                if(g.ghost[i].mode == chase)
                {
                    // Calcula a distância de Manhattan do Clyde ao Pac-Man
                    // Usamos abs() para garantir valores positivos
                    dy = fabs((float)g.pacman.pos.y - g.ghost[i].pos.y);
                    dx = fabs((float)g.pacman.pos.x - g.ghost[i].pos.x);

                    // A regra do Clyde:
                    // Se a distância total (dx + dy) for maior que 8, ele persegue o Pac-Man.
                    // Caso contrário (está a 8 ou menos blocos de distância), ele vai para seu canto de Scatter.
                    if((dx + dy) > 8) // Se "longe" do Pac-Man, persegue o Pac-Man
                    {
                        g.ghost[i].starget.y = g.pacman.pos.y;
                        g.ghost[i].starget.x = g.pacman.pos.x;
                    }
                    else // Se "perto" do Pac-Man (distância <= 8), vai para seu canto de Scatter
                    {
                        g.ghost[i].starget.y = 22; // Canto inferior esquerdo (seu target de scatter)
                        g.ghost[i].starget.x = 0;
                    }
                }
                else if(g.ghost[i].mode == scatter)
                {
                    g.ghost[i].starget.y = 22; // Canto inferior esquerdo
                    g.ghost[i].starget.x = 0;
                }
                else if(g.ghost[i].mode == dead)
                {
                    // Alvo para a "casa" dos fantasmas (posição inicial na casa)
                    g.ghost[i].starget.y = 11; // Posição inicial do Clyde na casa
                    g.ghost[i].starget.x = 10;

                    // Se Clyde chegou na casa, ele volta para o modo CHASE
                    if(g.ghost[i].pos.y == g.ghost[i].starget.y && g.ghost[i].pos.x == g.ghost[i].starget.x)
                    {
                        g.ghost[i].mode = chase;
                        // Certifique-se de que o timer de "afraid" global seja resetado
                        // ou a lógica de modo global seja desativada aqui, se aplicável.
                        // TempoAfraid = 0; // Se TempoAfraid é um sinalizador para todos
                    }
                    // Não use 'continue' aqui, pois ele precisa continuar se movendo em direção à casa
                }
                else if(g.ghost[i].mode == afraid)
                {
                    // Clyde se move aleatoriamente no modo AFRAID.
                    // É crucial garantir que ele não se mova para uma parede.
                    int next_y = g.ghost[i].pos.y;
                    int next_x = g.ghost[i].pos.x;
                    int valid_move_found = 0;
                    int attempts = 0;

                    // Tenta algumas vezes encontrar um movimento válido para evitar paredes
                    while (!valid_move_found && attempts < 10) {
                        int random_direction = rand() % 4; // 0: cima, 1: baixo, 2: esquerda, 3: direita
                        ...
                        switch (random_direction) {
                            case 0: // Cima
                                if (g.lab[next_y - 1][next_x] != '#' && g.lab[next_y - 1][next_x] != '-') {
                                    next_y--;
                                    valid_move_found = 1;
                                }
                                break;
                            case 1: // Baixo
                                ...
                                break;
                            case 2: // Esquerda
                                if (g.lab[next_y][next_x - 1] != '#' && g.lab[next_y][next_x - 1] != '-') {
                                    next_x--;
                                    valid_move_found = 1;
                                }
                                break;
                    ...
                    // Se encontrou um movimento válido, define como alvo.
                    // Se não, permanece na posição atual (o IAfantasmas moverá ele para lá).
                    if (valid_move_found) {
                        g.ghost[i].starget.y = next_y;
                        g.ghost[i].starget.x = next_x;
                    } else {
                        g.ghost[i].starget.y = g.ghost[i].pos.y;
                        g.ghost[i].starget.x = g.ghost[i].pos.x;
                    }
                }
                break; // Sai do switch(i) para o Clyde
            }
...
/* ---------------------------------------------------------------------- */
/*vii: set ai et ts=4 sw=4 tw=0 wm=0 fo=croql : C config for Vim modeline */
