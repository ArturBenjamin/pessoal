...
typedef enum e_ghostmode {chase, frightened, afraid, dead} t_ghostmode;
...
typedef struct st_pacman
{
    ...
    t_pos start; /* posiçao inicial do pacman */
} t_pacman;
...
typedef struct st_pacdata
{
    ...
    t_timer timer; //tempo do jogo
    t_pos cereja; //posição da cereja
    t_timer cereja_timer; //tempo de aparição da cereja
    t_timer cereja_espera; //controla a aparição da cereja após ela desaparecer
    int cereja_apareceu; //verifica se a cereja apareceu
    int cereja_apareceu_na_fase; //verifica se a cereja apareceu na fase
    int cereja_foi_coletada; //verifica se a cereja foi coletada
    t_timer power_pill_timer; //Timer para duração do modo afraid
    bool power_pill_active; // Saber se o modo está ativo
} t_game;
...
/* ---------------------------------------------------------------------- */
/* Menu ASCII Art */
const char menu_title[5][60] = {
    "  _    ____  _____   ____   __  __    _    _   _ ",
    " | |  |  _ \\| ____| |  __\\ |  \\/  |  / \\  | \\ | |",
    " | |  | |_) |  _|   | |    | |\\/| | / _ \\ |  \\| |",
    " | |__|  __/| |___  | |___ | |  | |/ ___ \\| |\\  |",
